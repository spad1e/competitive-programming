...
void solve() {
  ...
  rep(i, 1, n) {
    int op, x; cin >> op >> x;
    if (idx > 100) continue;
    if (op == 1) {a[idx].sz++, a[idx].c.pb(x);} 
    else {idx++, a[idx].lst = a[idx-1].sz, a[idx].sz = min((__int128)lim, (__int128)(a[idx-1].sz)*(x+1));}
  }
  while (q--) {
    ll x; cin >> x;
    repd(i, 0, idx) {
      if (x > a[i].sz - sz(a[i].c)) {
        cout << a[i].c[x - (a[i].sz - sz(a[i].c)) - 1] << " "; break;
      }
      x = x%a[i].lst; 
      if (x==0) x = a[i].lst;
    }
